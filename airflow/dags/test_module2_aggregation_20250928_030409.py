...
Создан автоматически: 2025-09-28T03:04:09.216854
Пайплайн: test_module2_aggregation
"""
...
tasks['create_test_scenario'] = PythonOperator(
    task_id='create_test_scenario',
    python_callable=execute_module_call,
    op_kwargs={
        'module': 2,
        'endpoint': '/api/v1/aggregation/create-scenario',
        'method': 'POST',
        'parameters': {"aggregations": [{"parameters": {"aggregates": {"count_records": "COUNT(id)", "total_value": "SUM(value)"}, "columns": ["category"]}, "type": "group_by"}], "description": "Test aggregation for small file", "name": "test_small_aggregation", "sources": [{"path": "/data/raw/test_small.csv", "selected_columns": ["id", "value", "category"], "source_id": "test_small_data", "source_type": "file"}], "target_requirements": {"expected_volume": "small", "performance_priority": "balanced"}}
    },
    retries=1,
