...
# Рекомендации для PostgreSQL (низкое качество данных -> PostgreSQL для OLTP)
recommendations = [
    ('/data/raw/sales_postgres.csv', 'index', '{"columns": ["customer_id", "product_name"]}'),
    ('/data/raw/sales_postgres.csv', 'partition', '{"partition_by": "RANGE (sale_date)"}'),
]
...
try:
    ...
    with psycopg2.connect(**db_params) as conn:
        with conn.cursor() as cur:
            print("Inserting/updating PostgreSQL recommendations...")
            ...
            conn.commit()
            print(f"[SUCCESS] {len(recommendations)} PostgreSQL optimization recommendations inserted.")
