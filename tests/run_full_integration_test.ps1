...
Write-Host "`n[Step 0] Environment preparation..." -ForegroundColor Cyan
...
foreach ($service in $services) {
    if ($service.url) {
        try {
            ...
            Write-Host "[ERROR] $($service.name) is not responding at $($service.url)" -ForegroundColor Red
            throw "Service $($service.name) is not available"
        }
    }
}

# --- Шаг 1: Подготовка тестовых данных ---
Write-Host "`n[Step 1] Preparing comprehensive test data..." -ForegroundColor Cyan
...
Write-Host "`n[Step 3] Starting warehouse design process..." -ForegroundColor Cyan
$designBody = @{
...
Write-Host "`n[Step 4] Confirming design and generating DDL..." -ForegroundColor Cyan
$confirmBody = @{
...
Write-Host "`nGenerated DDL Script:"
Write-Host "====================="
...
Write-Host "`n[Step 5] Creating tables in ClickHouse..." -ForegroundColor Cyan
try {
...
Write-Host "`n[Step 6] Verifying created tables..." -ForegroundColor Cyan
try {
...
Write-Host "`n[Step 7] Generating and triggering ETL pipeline..." -ForegroundColor Cyan
try {
...
Write-Host "`n[Step 8] Checking loaded data..." -ForegroundColor Cyan
...
Write-Host "`n[Step 9] Checking warehouse metrics..." -ForegroundColor Cyan
try {
...
# --- Финальная проверка ---
Write-Host "`n[Final Check] Comprehensive validation..." -ForegroundColor Cyan
...
Write-Host "`n=== FULL INTEGRATION TEST RESULTS ===" -ForegroundColor Yellow
Write-Host "Passed checks: $passed_checks / $($checks.Count)"

if ($passed_checks -eq $checks.Count) {
    Write-Host "`n--- FULL Integration Test for Module 4 PASSED ---" -ForegroundColor Green
    ...
    Write-Host "   • Intelligent SUBД selection with LLM" -ForegroundColor Green
    ...
    Write-Host "`n--- FULL Integration Test for Module 4 PARTIALLY PASSED ---" -ForegroundColor Yellow
    Write-Host "Some components may need additional time or configuration" -ForegroundColor Yellow
}

Write-Host "`nTest completed. Check Airflow UI at http://localhost:8080 for DAG execution details." -ForegroundColor Cyan
