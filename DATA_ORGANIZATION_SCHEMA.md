...
└── exports/                  # Экспорты метаданных
    ...
    ├── external_api_sources.json # Справочник внешних API
    └── system_health.json
```

### **5. 🎭 ЭМУЛЯЦИЯ ВНЕШНИХ API (FAKE API SERVER)**

Для целей тестирования, разработки и демонстрации в систему встроен "фальшивый" API-сервер (`fake_external_api.py`). Он позволяет имитировать работу внешних API-источников без необходимости реального подключения.

**Ключевые особенности:**

-   **Назначение**: Эмуляция внешних API для обнаружения и анализа данных модулем `DataTypeDetector`.
-   **Источник данных**: Сервер читает синтетические данные из CSV-файлов, расположенных в `data_landing_zone/raw/external/api_sources/`.
-   **Принцип работы**: При запуске он предоставляет HTTP-эндпоинты, соответствующие записям в `external_api_sources.json`. По этим эндпоинтам он отдает метаданные (схемы таблиц) и примеры данных.
-   **Интеграция с Docker**: Сервер автоматически запускается как сервис `fake-api` в `docker-compose.yml`. Внутри Docker-сети он доступен по адресу `http://fake-api:8001`.
-   **Гибкость**: Для переключения на реальный внешний API не требуется изменять код. Достаточно добавить или изменить соответствующую запись в файле `external_api_sources.json`, указав реальный URL. Для временного отключения источника можно использовать флаг `"enabled": false`.
